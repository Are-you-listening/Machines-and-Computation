...
class TMBuilder {
public:
    explicit TMBuilder(unsigned int tuple_size);
    json generateTM();
private:
    unsigned int tapes;
    TuringTools* tools;

    Transition make_transition(IncompleteTransition& inComp) const;
    static json add_transition(Transition& transition);
};
